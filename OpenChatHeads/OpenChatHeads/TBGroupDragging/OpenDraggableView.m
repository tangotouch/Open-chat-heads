...
@implementation OpenDraggableView
...
-(BOOL)isEqual:(id)object{
    if (![object isKindOfClass:[OpenDraggableView class] ]) {
        return NO;
    }
    OpenDraggableView *obj =(OpenDraggableView *)object;
    return [self.uniqueIdentifier isEqual:obj.uniqueIdentifier];
}
...
{
    BOOL canMove=   [_delegate draggableViewCanMove:self];
    if (canMove) {
        [CATransaction begin];
        UITouch *touch = [touches anyObject];
...
- (void)snapViewCenterToPoint:(CGPoint)point edge:(CGRectEdge)edge completion:(void (^)(BOOL finished))completion{
    [self _snapViewCenterToPoint:point edge:edge completion:completion];
}

- (void)_snapViewCenterToPoint:(CGPoint)point edge:(CGRectEdge)edge completion:(void (^)(BOOL finished))completion
{
...
{
    [self _snapViewCenterToPoint:point edge:edge completion:nil];
}
